...
public class DialView extends View {
    ...
    final private static int SELECTION_COUNT = 4;  // Total number of selections.
    private static final int INITIAL_DIAL_COLOR = Color.BLUE;
    ...
    private Paint mDotPaint;                 // For the dot
    ...
    private float mSelectionPosition;
    ...
    private void init() {
        ...
        mDialPaint.setColor(INITIAL_DIAL_COLOR);
...
//        });
        ...
        setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                Log.v("fan", "motion event, x: " + motionEvent.getX());
                Log.v("fan", "motion event, y: " + motionEvent.getY());
                Log.v("fan", "motion action, x: " + motionEvent.getAction());
                if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                    return true;
                }

                mSelectionPosition = (getWidth() / motionEvent.getX()) * SELECTION_COUNT;
                // ignore if touch point is outsite of circle
                // calculate new position of dot
                // - don't let dot be lower than 0 or greater than 3
                // Rotate selection forward to the next valid choice.
                mActiveSelection = (mActiveSelection + 1) % SELECTION_COUNT;
                // Set dial background color to green if selection is >= 1.
                if (mActiveSelection >= 1) {
                ...
                // Redraw the view.
                invalidate();
                // can we just redraw what changed?
                // https://developer.android.com/reference/android/view/View.html#invalidate%28android.graphics.Rect%29
                return true;
            }
        });
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < SELECTION_COUNT; i++) {
            float[] xyData = computeXYForPositionForLabel(i, labelRadius);
            float x = xyData[0];
            float y = xyData[1];
            label.setLength(0);
            label.append(i);
        ...
        float[] xyData = computeXYForPositionForDot(mSelectionPosition, markerRadius);
        float x = xyData[0];
        float y = xyData[1];
        canvas.drawCircle(x, y, 20, mDotPaint);
    }
     ...
     */
    private float[] computeXYForPositionForLabel(final int pos, final float radius) {
    ...
    private float[] computeXYForPositionForDot(final float pos, final float radius) {
        float[] result = mTempResult;
        Double startAngle = Math.PI * (9 / 8d);   // Angles are in radians.
        Double angle = startAngle + (pos * (Math.PI / 4));
        result[0] = (float) (radius * Math.cos(angle)) + (mWidth / 2);
